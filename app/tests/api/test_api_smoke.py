...
async def test_create_and_list_people(client: AsyncClient) -> None:
    resp = await client.post("/people", json={"name": "Alice", "seniority": "P2"})
...
async def test_add_observation(client: AsyncClient) -> None:
    person_resp = await client.post("/people", json={"name": "Bob"})
...
async def test_log_one_on_one(client: AsyncClient) -> None:
    person_resp = await client.post("/people", json={"name": "Carla"})
    person_id = person_resp.json()["id"]
...
async def test_complete_action_item(client: AsyncClient) -> None:
    person_resp = await client.post("/people", json={"name": "Dimitri"})
    person_id = person_resp.json()["id"]
    ...
    items_resp = await client.get(f"/action-items?person_id={person_id}")
    items = items_resp.json()
    assert len(items) == 1

    complete_resp = await client.post(f"/action-items/{items[0]['id']}/complete")
    assert complete_resp.status_code == 200
    assert complete_resp.json()["status"] == "done"


async def test_update_allocation(client: AsyncClient) -> None:
    person_resp = await client.post("/people", json={"name": "Eve"})
    person_id = person_resp.json()["id"]
